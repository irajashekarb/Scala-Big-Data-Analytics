...
object TempData {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Finding highest temperature recorded in the data using Higher order funtions

    // Inefficient way of finding max temperature from the data
    val maxTemp = data.map(_.tmax).max
    val hotDays = data.filter(_.tmax == maxTemp)
    println(s"Hot days are ${hotDays.mkString(", ")}")

    // Efficient way of finding max temperature from the data
    val hotDay = data.maxBy(_.tmax)
    println(s"Hot day 1 is $hotDay")

    val hotDay2 = data.reduceLeft((d1, d2) => if(d1.tmax >= d2.tmax) d1 else d2)
    println(s"Hot day 2 is $hotDay")

    // Days that have one inch or more precipitation
